...
<head>
    <meta charset="UTF-8">
    <title>Nueva Nota de Compra - TesisGyA</title>
    ...
    <div class="main-content">
        <div class="content-wrapper">
            ...
            <div class="page-header">
                <div>
                    <h2><i class="fas fa-file-alt"></i> Nueva Nota de Compra</h2>
                    <small>Registrar nota de crédito o débito de proveedor</small>
                </div>
            ...
            <form action="{{ route('notas_compra.store') }}" method="POST" id="notaCompraForm">
                @csrf
                    ...
                    <div class="card-body">
                        <div class="form-grid-2">
                            ...
                            <div>
                                ...
                                <select class="form-select form-select-sm" id="factura_id" name="factura_id" required disabled>
                                    <option value="">Seleccione primero un proveedor...</option>
                                </select>
                                <small class="text-muted">Se listan las facturas del proveedor seleccionado</small>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="tipo-doc-grid">
                            @foreach($tiposDocumento as $tipo)
                                <label class="tipo-doc-option {{ old('tipo_documento_id') == $tipo->id ? 'selected' : '' }}">
                                    <input type="radio" name="tipo_documento_id" value="{{ $tipo->id }}"
                                           {{ old('tipo_documento_id') == $tipo->id ? 'checked' : '' }} required>
                            ...
                            <div>
                                ...
                                <select class="form-select form-select-sm @error('iva_id') is-invalid @enderror"
                                    ...
                                    @foreach($impuestos as $imp)
                                        <option value="{{ $imp->id }}" {{ old('iva_id') == $imp->id ? 'selected' : '' }}>
                                            {{ $imp->descripcion }}
                                        </option>
                                    @endforeach
                                </select>
...
// Máscara para nro_nota: auto insertar guiones XXX-XXX-XXXXXXX
