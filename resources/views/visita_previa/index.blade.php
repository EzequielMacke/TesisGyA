    ...
    <div class="main-content fade-in">
        <div class="content-wrapper">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="text-primary"><i class="fas fa-clipboard-list me-2"></i>Visitas Previas</h2>
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="collapse" id="filtrosCollapse">
                    <div class="card-body">
                        <form method="GET" action="{{ route('visita_previa.index') }}" class="row g-3">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" name="solicitud_id" id="solicitud_id" class="form-control" value="{{ request('solicitud_id') }}" placeholder="Buscar por ID">
                            </div>
                            <div class="col-md-12">
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-light">
                    <h5 class="mb-0"><i class="fas fa-list me-2"></i>Lista de Visitas Previas</h5>
                </div>
                        ...
                        <table class="table table-hover mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th><i class="fas fa-file-alt me-1"></i>Solicitud</th>
                            ...
                            <tbody>
                                @forelse($visitas as $visita)
                                    <tr>
                                        ...
                                        <td>{{ $visita->obra->descripcion ?? '-' }}</td>
                                        <td>
                                            @if($visita->solicitud_servicio_id)
                                                <span class="badge bg-info">#{{ $visita->solicitud_servicio_id }}</span>
                                            @else
                                        ...
                                        <td>{{ $visita->usuario->usuario ?? '-' }}</td>
                                        <td>
                                            @if($visita->observacion)
                                                <span title="{{ $visita->observacion }}">{{ Str::limit($visita->observacion, 30) }}</span>
                                            @else
                                                <span class="text-muted">-</span>
                                            @endif
                                        </td>
                                        <td class="btn-actions">
                                            <a href="{{ route('visita_previa.show', $visita->id) }}" class="btn btn-sm btn-outline-info me-1" title="Ver detalles">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="#" class="btn btn-sm btn-outline-warning me-1" title="Editar">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="#" method="POST" class="d-inline" onsubmit="return confirm('¿Estás seguro de eliminar esta visita previa?')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="9" class="empty-state">
                                            ...
                                            <p>No hay visitas previas registradas.</p>
                                            <a href="{{ route('visita_previa.create') }}" class="btn btn-primary">
