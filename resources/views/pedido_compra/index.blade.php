                        ...
                        <div class="toolbar-item search-item">
                            ...
                            <div class="search-box">
                                ...
                                <input type="text" class="form-control form-control-sm" id="searchInput"
                                       placeholder="Código, usuario, sucursal, depósito..." autocomplete="off">
                            ...
                            <table id="pedidosTable">
                                ...
                                <tbody>
                                    @foreach($pedidos as $pedido)
                                        <tr class="pedido-row"
                                            data-estado="{{ $pedido->estado_id }}"
                                            data-sucursal="{{ $pedido->sucursal_id }}"
                                            ...
                                            <td>
                                                <span class="cell-text" title="{{ $pedido->usuario->usuario }}">
                                                    {{ Str::limit($pedido->usuario->usuario, 18) }}
                                                </span>
                                            </td>
                                            <td>
                                                <span class="cell-text" title="{{ $pedido->sucursal->descripcion }}">
                                                    {{ Str::limit($pedido->sucursal->descripcion, 18) }}
                                                </span>
                                            </td>
                                            <td>
                                                <span class="cell-text" title="{{ $pedido->deposito->descripcion }}">
                                                    {{ Str::limit($pedido->deposito->descripcion, 18) }}
                                                </span>
                                            </td>
                                            <td class="text-center">
    ...
    if (table) {
        ...
        function applyAllFilters() {
            ...
            rows.forEach(row => {
                ...
                if (sucursal && row.dataset.sucursal !== sucursal) show = false;
                ...
                if (term && show) {
                    ...
                    for (let i = 0; i < cells.length - 1; i++) {
                        if (cells[i].textContent.toLowerCase().includes(term)) {
                            found = true;
                            if (!cells[i].querySelector('.estado')) {
