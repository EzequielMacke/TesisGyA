...
<head>
    <meta charset="UTF-8">
    <title>Editar Insumos Utilizados</title>
    ...
    <div class="main-content">
        <div class="content-wrapper">
            ...
            <div class="page-header">
                <div>
                    <h2><i class="fas fa-boxes"></i> Editar Insumos Utilizados Nro {{ $insumoUtilizado->nro }}</h2>
                    <small>Solo se pueden modificar los insumos cargados y la observación</small>
                </div>
            ...
            <form action="{{ route('insumos_utilizados.update', $insumoUtilizado->id) }}" method="POST" id="insumoUtilizadoForm">
                @csrf
                @method('PUT')
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="detail-box">
                            <div class="detail-box-title">Datos de la Orden</div>
                            ...
                            <div class="detail-row"><i class="fas fa-building"></i><span><strong>Cliente:</strong> {{ $insumoUtilizado->ordenServicio->obra->cliente->razon_social ?? '-' }}</span></div>
                            <div class="detail-row"><i class="fas fa-map-marker-alt"></i><span><strong>Obra:</strong> {{ $insumoUtilizado->ordenServicio->obra->descripcion ?? '-' }}</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="alert alert-warning py-2 px-3 mb-3" id="sin-inventario-alert" style="display:none; font-size:0.82rem;">
                            <i class="fas fa-exclamation-triangle me-2"></i>La obra de esta orden no tiene insumos en inventario.
                        </div>
                        <div class="add-grid">
                            <div>
                                <label for="insumo_select" class="form-label">Insumo</label>
                                <select class="form-select form-select-sm" id="insumo_select">
                ...
                <div class="card table-card">
                    <div class="card-header-section">
                        ...
                        <span class="results-count" id="totalItems">{{ $detallesIniciales->count() }} items</span>
                    </div>
                            ...
                            <table id="insumosTable">
                                ...
                                <tbody id="insumosBody">
                                    @foreach($detallesIniciales as $index => $detalle)
                                        <tr>
                                            <td>
                                                <i class="fas fa-cube text-muted me-2"></i><strong>{{ $detalle['descripcion'] }}</strong>
                                                <input type="hidden" name="insumos[{{ $index }}][insumo_id]" value="{{ $detalle['id'] }}">
                                            </td>
                                            <td>
                                                <span class="tag">{{ $detalle['unidad'] }}</span>
                                            </td>
                                    ...
                                    @endforeach
                                    <tr id="emptyRow" style="{{ $detallesIniciales->count() > 0 ? 'display:none;' : '' }}">
                    ...
                    <div class="card-body">
                        <div class="form-grid">
                            <div>
                                ...
                                <input type="text" id="nro_display" class="form-control form-control-sm readonly-field" value="{{ $insumoUtilizado->nro }}" readonly>
                            </div>
                            <div>
                                <label for="fecha_registro_display" class="form-label">Fecha de Registro</label>
                                <input type="text" id="fecha_registro_display" class="form-control form-control-sm readonly-field" value="{{ $insumoUtilizado->fecha_registro->format('d/m/Y') }}" readonly>
                            </div>
                            <div>
                                <label for="usuario_display" class="form-label">Usuario</label>
                                <input type="text" id="usuario_display" class="form-control form-control-sm readonly-field" value="{{ $insumoUtilizado->usuario->usuario ?? '-' }}" readonly>
                            </div>
                            <div class="span-4">
                                <label for="observacion" class="form-label">Observación</label>
                                <textarea name="observacion" id="observacion" class="form-control form-control-sm" rows="2" placeholder="Ingrese una observación...">{{ old('observacion', $insumoUtilizado->observacion) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="text-center mt-3">
                    <button type="submit" class="btn btn-success" id="guardarInsumoUtilizado" {{ $detallesIniciales->count() > 0 ? '' : 'disabled' }}>
                        <i class="fas fa-save me-2"></i>Actualizar Insumos Utilizados
                    </button>
                </div>
            </form>
...
$(document).ready(function() {
    ...
    let insumosAgregados = @json($detallesIniciales->pluck('id')->values());
    let insumoCounter = {{ $detallesIniciales->count() }};
    const insumosDisponibles = @json($insumosDisponibles);
